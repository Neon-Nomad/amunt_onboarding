...
if (!API_KEY) {
    // As per instructions, we assume the key is always present.
    console.error("API_KEY is not configured.");
}

// Add a non-null assertion `!` because the environment is expected to have the API_KEY.
...
const textToStream = (result: AsyncIterable<GenerateContentResponse>): ReadableStream<Uint8Array> => {
...
export const startChatbotStream = async (history: ChatMessage[], pageContext?: string): Promise<ReadableStream<Uint8Array>> => {
    const model = 'gemini-2.5-flash';
